...
module.exports.login = async (req, res) => {
  ...
  if (comparedPwd) {
    let token = jwt.sign({ user_id: loginUser.id }, ".....", {
      expiresIn: "2h",
    });
...
module.exports.register = async (req, res) => {
  ...
  if (existedUser.length !== 0)
    return res.status(404).json({ msg: "Email or username already existed!" });
  ...
  newUser
    ...
    .then((data) => {
      const courier = CourierClient({
        authorizationToken: "pk_prod_EA07M5YCKF4QX9PRT8SCQAN9EN0Z",
      });
      courier
        .send({
          message: {
      ...
      let token = jwt.sign({ user_id: data._id }, ".....", { expiresIn: "2h" });
